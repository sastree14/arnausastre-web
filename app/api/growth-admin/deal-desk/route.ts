...
function jsonSection(form:FormData,names:string[]){return Object.fromEntries(names.map((name)=>[name,text(form,name)]).filter(([,value])=>value!==''))}
...
export async function POST(request:Request){
  ...
  if(action==='create'){
    ...
    const opportunity=await insertGrowthRow<Row>('crm_opportunities',{opportunity_id:opportunityId,tenant_id:'sc-analytics',company_id:companyId,primary_person_id:primaryPersonId,name:text(form,'name')||`${company.name} · ${recommendedOffer||'Oportunidad'}`,stage:'qualification',value:0,currency:text(form,'currency')||'EUR',probability:15,source:'deal_desk',next_action_at:null,metadata:{recommended_offer:company.recommended_offer||null,recommended_service:company.recommended_service||null,created_from:'deal_desk'},created_at:now,updated_at:now})
  ...
  if(action==='qualification'){
    ...
    payload={...jsonSection(form,['problem','impact','decision_maker','budget_range','data_readiness','qualification_notes']),problem_score:scores[0],impact_score:scores[1],urgency_score:scores[2],authority_score:scores[3],budget_score:scores[4],data_score:scores[5],qualification_score:qualificationScore}
    ...
    const nextActionAt=text(form,'next_action_at');if(nextActionAt) payload.next_action_at=nextActionAt
    opportunityChanges={...opportunityChanges,stage:'discovery',probability:40,next_action_at:nextActionAt||null}
  }else if(action==='proposal'){
    section='proposal';payload=jsonSection(form,['objective','scope','deliverables','exclusions','timeline','acceptance_criteria','assumptions','proposal_notes']);opportunityChanges={...opportunityChanges,stage:'proposal',probability:60}
  }else if(action==='budget'){
    section='budget'
    const estimatedHours=Math.max(0,number(form,'estimated_hours')),internalRate=Math.max(0,number(form,'internal_rate')),externalCosts=Math.max(0,number(form,'external_costs')),contingencyPct=Math.max(0,number(form,'contingency_pct',10)),targetMarginPct=Math.max(0,Math.min(95,number(form,'target_margin_pct',35)))
    const baseCost=estimatedHours*internalRate+externalCosts,contingency=baseCost*(contingencyPct/100),costWithContingency=baseCost+contingency,suggestedPrice=targetMarginPct>=95?costWithContingency:costWithContingency/(1-targetMarginPct/100),finalPrice=Math.max(0,number(form,'final_price',Math.round(suggestedPrice*100)/100))
    ...
    const stage=text(form,'stage'),probabilities:Record<string,number>={qualification:20,discovery:40,discovery_booked:45,proposal:60,negotiation:80,won:100,lost:0}
  ...
  }else return new NextResponse('Unsupported deal desk action',{status:400})
