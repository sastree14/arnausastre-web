...
function approvalAction(contentType:string){if(contentType==='linkedin_article')return'publish_linkedin_article';if(contentType==='article'||contentType==='web_article')return'publish_article';return'publish_post'}
export async function POST(request:Request){
  ...
  const actionType=approvalAction(String(item.content_type||''))
  ...
  const now=new Date().toISOString(),payload={...(approval?.payload||{}),manual_override:manualOverride,automatic_issues:gate?.readiness.issues||[],automatic_review:item.critique||{},decided_from:'editorial_workspace',execution_mode:actionType==='publish_linkedin_article'?'manual_linkedin_article':approval?.payload?.execution_mode}
