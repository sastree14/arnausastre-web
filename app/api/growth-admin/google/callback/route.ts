...
export async function GET(request: Request) {
  ...
  try {
    ...
    const existing = await queryGrowthTable<Row>('integration_connections', {
      tenant_id: 'eq.sc-analytics',
      provider: 'eq.gmail',
      provider_subject: `eq.${email}`,
      limit: '1',
    }, { cacheSeconds: 0 })
    const now = new Date().toISOString()
    const expiresAt = new Date(Date.now() + Number(token.expires_in || 3600) * 1000).toISOString()
    const changes: Record<string, unknown> = {
      account_type: String(stateData.accountType || 'corporate'),
      provider_subject: email,
      display_name: email,
      access_token_ciphertext: encryptGoogleToken(String(token.access_token)),
      token_expires_at: expiresAt,
      scopes: String(token.scope || 'openid email https://www.googleapis.com/auth/gmail.readonly').split(' '),
      metadata: { email, history_id: profile.historyId || '', messages_total: profile.messagesTotal || 0, threads_total: profile.threadsTotal || 0 },
      updated_at: now,
    }
    if (token.refresh_token) changes.refresh_token_ciphertext = encryptGoogleToken(String(token.refresh_token))

    if (existing[0]) {
      await updateGrowthRow('integration_connections', 'connection_id', String(existing[0].connection_id), changes)
    } else {
      await insertGrowthRow('integration_connections', {
    ...
    return NextResponse.redirect(new URL('/growth-admin/finance/integrations?google_connected=1', request.url))
  } catch (error) {
