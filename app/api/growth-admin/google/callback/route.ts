...
export async function GET(request: Request) {
  ...
  try {
    ...
    const changes: Record<string, unknown> = {
      ...
      scopes,
      metadata: { ...previousMetadata, email, history_id: profile.historyId || '', messages_total: profile.messagesTotal || 0, threads_total: profile.threadsTotal || 0, disconnected: false, account_role: accountType },
      updated_at: now,
    }
    if (token.refresh_token) changes.refresh_token_ciphertext = encryptGoogleToken(String(token.refresh_token))
    else if (!existing[0]?.refresh_token_ciphertext) throw new Error('Google did not return a refresh token. Reconnect and accept consent.')
    ...
    next.searchParams.set('google_connected', '1')
    next.searchParams.set('account', email)
    return NextResponse.redirect(next)
  } catch (error) {
    console.error('Google OAuth callback failed', error)
    const message = error instanceof Error ? error.message : String(error)
    const next = new URL('/growth-admin/finance/integrations', request.url)
