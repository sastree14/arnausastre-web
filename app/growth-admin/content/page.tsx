      ...
      {filtered.map((item) => {
        ...
        return <article key={item.content_id} className={`${adminPanel} overflow-hidden`}>
          <div className="grid xl:grid-cols-[210px_1fr]">
            <div className="border-b border-slate-200 bg-slate-50 p-4 xl:border-b-0 xl:border-r">{image ? <a href={`/growth-admin/preview/${item.content_id}`}><img src={image} alt={item.title} className="h-40 w-full rounded-xl border border-slate-200 bg-white object-contain"/></a> : <div className="flex h-40 items-center justify-center rounded-xl border border-dashed border-slate-300 bg-white px-4 text-center text-xs text-slate-400">Sin visual adjunto</div>}<a href={`/growth-admin/visual-studio?content=${encodeURIComponent(item.content_id)}`} className="mt-3 block rounded-lg border border-indigo-200 bg-white px-3 py-2 text-center text-xs font-semibold text-indigo-700 transition hover:bg-indigo-50">{image?'Editar visual':'Diseñar visual'}</a></div>
