  ...
  return <AdminShell active="finance">
    ...
    <section className="mt-14 pb-12"><SectionHeading eyebrow="Factura · Paso 2 y 3" title="Revisar y emitir" description="Una factura emitida no se edita silenciosamente. Si hay un error posterior, debe rectificarse y conservar trazabilidad." count={bundle.invoices.length}/>
      {bundle.invoices.length===0?<EmptyState>No hay facturas registradas.</EmptyState>:<div className="space-y-3">{bundle.invoices.map((invoice)=>{
        ...
        const paid=confirmedPayments.reduce((sum,payment)=>sum+n(payment.amount_eur??payment.amount),0)
        const due=n(invoice.amount_eur??invoice.total),remaining=Math.max(0,Math.round((due-paid)*100)/100)
        return <div key={invoice.invoice_id} className={`${adminPanel} p-5`}><div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between"><div><div className="flex flex-wrap items-center gap-2"><p className="font-semibold text-slate-950">{invoice.invoice_number||'Borrador sin numerar'}</p><Badge tone={statusTone(String(invoice.status||''))}>{String(invoice.status||'draft')}</Badge><Badge tone={reviewed?'green':'amber'}>{String(invoice.review_status||'pending_review')}</Badge></div><p className="mt-2 text-xs text-slate-500">{invoice.issue_date||'Sin fecha'} · {invoice.recipient_legal_name||'Cliente'} · {n(invoice.total).toLocaleString('es-ES',{minimumFractionDigits:2})} {invoice.currency||'EUR'}</p><p className="mt-1 text-[11px] text-slate-400">{invoice.tax_rule_key||invoice.tax_profile||'Fiscalidad pendiente'} · IVA {n(invoice.vat_rate)}% · retención {n(invoice.withholding_rate)}%</p>{issued&&<p className="mt-2 text-xs font-medium text-slate-600">Cobrado confirmado: {paid.toLocaleString('es-ES',{minimumFractionDigits:2})} € · Pendiente: {remaining.toLocaleString('es-ES',{minimumFractionDigits:2})} €</p>}</div><div className="flex flex-wrap gap-2">
        ...
        {issued&&remaining>0&&<form action="/api/growth-admin/finance" method="post" data-live-form="1" className="mt-4 grid gap-2 rounded-xl border border-emerald-100 bg-emerald-50/60 p-4 md:grid-cols-[150px_160px_170px_1fr_auto]">
          ...
          {String(invoice.currency||'EUR')!=='EUR'&&<label className="text-[10px] font-semibold uppercase tracking-wide text-emerald-800 md:col-span-2">Cambio a EUR<input required type="number" step="0.000001" min="0" name="fx_rate" placeholder="Ej. 0.86" className={`mt-1 w-full ${adminInput}`}/></label>}
        </form>}
