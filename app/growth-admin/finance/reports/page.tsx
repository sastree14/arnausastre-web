...
export default async function ReportsPage({searchParams}:{searchParams:Promise<{kind?:string;anchor?:string;closure?:string}>}){
  ...
  const chartRows=[
    ['Facturado',n(current.invoiced),n(previous.invoiced)],
    ['Gastos',n(current.expenses),n(previous.expenses)],
    ['Resultado',result,previousResult],
    ['Cobros',n(current.cash_in),n(previous.cash_in)],
    ['Pagos',n(current.cash_out),n(previous.cash_out)],
  ] as const
