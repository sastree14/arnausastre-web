...
import { getOperationsBundle } from '@/lib/growth-admin-performance'
...
type Opportunity={opportunity_id:string;name:string}
...
export default async function OperationsPage(){
  ...
  const opportunities = bundle.opportunities as Opportunity[]
  ...
  return <AdminShell active="operations">
    ...
    {bundle.degraded && <div className="mb-6 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm text-amber-800">Operaciones ha cargado en modo seguro porque el backend tardó demasiado. La interfaz no se bloquea y puedes refrescar para recuperar el detalle.</div>}
    ...
    <section id="knowledge" className="mt-14"><SectionHeading eyebrow="CRM · Knowledge" title="Documentación y herramientas" description="Knowledge, Drive, SOPs, manuales, plantillas e integraciones pertenecen a Operaciones aunque parte del trabajo siga siendo manual."/><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><a href="https://drive.google.com/" target="_blank" rel="noreferrer" className={`${adminPanel} p-5 transition hover:-translate-y-0.5 hover:shadow-md`}><p className="font-semibold text-slate-950">Google Drive ↗</p><p className="mt-2 text-sm leading-6 text-slate-500">Repositorio documental y knowledge operativo.</p></a><div className={`${adminPanel} p-5`}><p className="font-semibold text-slate-950">SOPs y manuales</p><p className="mt-2 text-sm leading-6 text-slate-500">Procesos repetibles, checklist, guías y documentación de delivery.</p></div><div className={`${adminPanel} p-5`}><p className="font-semibold text-slate-950">Plantillas y assets</p><p className="mt-2 text-sm leading-6 text-slate-500">Proposals, budgets, PPT, informes, entregables y materiales reutilizables.</p></div><a href="/growth-admin/system" className={`${adminPanel} p-5 transition hover:-translate-y-0.5 hover:shadow-md`}><p className="font-semibold text-slate-950">Integraciones</p><p className="mt-2 text-sm leading-6 text-slate-500">Estado de LinkedIn, GA4, Apollo, Calendly, workers y conectores.</p></a></div></section>
