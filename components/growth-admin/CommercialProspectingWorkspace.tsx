...
type WorkspaceMode = 'lead' | 'partner' | 'network'
...
function fallbackMessage(person: CrmPerson, company: CrmCompany | undefined, mode: WorkspaceMode) {
  ...
  if (mode === 'network') {
    return `Hola ${firstName}, soy Arnau Sastre, matemático y estadístico y fundador de SC-Analytics. Estoy intentando conocer mejor a gente que esté construyendo y trabajando de verdad en Data/AI, analítica y optimización. Me ha parecido interesante tu trabajo en ${companyName} y me gustaría seguir lo que haces y conectar, sin enfoque comercial. ¿En qué tipo de problemas o proyectos estás más metido ahora mismo?`
  }
...
function modeOfCompany(company: CrmCompany): WorkspaceMode {
  if (company.fit_type === 'partner') return 'partner'
  if (company.fit_type === 'network') return 'network'
  return 'lead'
}
...
export default async function CommercialProspectingWorkspace({ mode, searchParams }: { mode: WorkspaceMode; searchParams: Promise<SearchParams> }) {
  ...
  const returnTo = isPartner ? '/growth-admin/partners' : isNetwork ? '/growth-admin/network' : '/growth-admin/crm'
  ...
  return <AdminShell active={isPartner ? 'partners' : isNetwork ? 'network' : 'crm'} surface="light">
    <div className="pb-12">
      <header className="overflow-hidden rounded-[2rem] border border-slate-900 bg-slate-950 text-white shadow-sm">
        <div className="grid gap-8 px-6 py-9 md:px-8 md:py-11 xl:grid-cols-[1fr_auto] xl:items-end">
          <div className="max-w-4xl">
            <p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-indigo-300">Commercial OS · {isPartner ? 'Partnership engine' : isNetwork ? 'Professional network' : 'Client acquisition'}</p>
            <h1 className="mt-4 text-4xl leading-tight md:text-6xl" style={{ fontFamily: 'var(--font-playfair)' }}>{isPartner ? 'Partners estratégicos' : isNetwork ? 'Red profesional' : 'Clientes potenciales'}</h1>
            <p className="mt-5 max-w-3xl text-sm leading-7 text-slate-300 md:text-base">{isPartner
              ...
              : isNetwork
                ? 'Personas de Data, AI, analítica, optimización y tecnología con las que merece la pena construir relación. Aquí el objetivo no es vender: es aprender, aportar, ganar reconocimiento y estar dentro del ecosistema.'
          ...
          <form action="/api/growth-admin/operator-task" method="post" className="flex shrink-0 items-center gap-2 rounded-2xl border border-white/10 bg-white/5 p-2">
            ...
            <button className="rounded-xl bg-white px-5 py-3 text-sm font-semibold text-slate-950 shadow-sm transition hover:bg-slate-100">{isPartner ? 'Buscar partners' : isNetwork ? 'Buscar perfiles' : 'Buscar clientes'}</button>
          </form>
        </div>
        <div className="border-t border-white/10 bg-white/[0.035] px-6 py-4 md:px-8">
          <div className="flex flex-wrap gap-x-6 gap-y-2 text-xs text-slate-400">
            <span><strong className="font-semibold text-slate-200">{isNetwork ? 'Relación 1:1' : 'Contrato 1:1'}</strong> → {isPartner ? 'partner + decisor' : isNetwork ? 'persona + contexto profesional' : 'empresa + decisor'}</span>
            <span><strong className="font-semibold text-slate-200">LinkedIn</strong> → persona + empresa</span>
            <span><strong className="font-semibold text-slate-200">Personalización</strong> → {isNetwork ? 'trabajo público + interés común + pregunta abierta' : 'detalle profesional + oferta + pregunta abierta'}</span>
            {!isPartner&&!isNetwork&&<span><strong className="font-semibold text-slate-200">ICP tamaño</strong> → 10–200 ideal · 500 empleados máximo</span>}
            {isNetwork?<span><strong className="font-semibold text-slate-200">Objetivo</strong> → reconocimiento · aprendizaje · relaciones · colaboraciones futuras</span>:<span><strong className="font-semibold text-slate-200">Teléfono</strong> → solo si existe contacto empresarial público y aporta valor</span>}
          </div>
        </div>
      </header>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        <MetricCard label={isPartner ? 'Partners activos' : isNetwork ? 'Contextos activos' : 'Empresas activas'} value={activeCompanies.length} note={isNetwork?'Organizaciones o proyectos de referencia':'Cuentas por trabajar'}/>
        <MetricCard label={isNetwork?'Perfiles':'Decisores'} value={activePeople.length} tone="indigo" note={isNetwork?'Personas relevantes del ecosistema':'Una persona prioritaria por cuenta'}/>
        <MetricCard label={isNetwork?'Conectados / contactados':'Contactados'} value={contactedPeople.length} tone="amber" note="Seguimiento o mensaje iniciado"/>
        <MetricCard label="Conversaciones" value={conversationPeople.length} tone="sky" note={isNetwork?'Relación bidireccional iniciada':'Respuesta, follow-up o discovery'}/>
        <MetricCard label={isNetwork?'Reuniones':'Acuerdos / pipeline'} value={isNetwork?meetings.length:opportunities.length} tone="green" note={isNetwork?'Interacciones más profundas':'Discovery, propuesta o negociación'}/>
      </div>

      <section className="mt-10 rounded-3xl border border-indigo-100 bg-indigo-50/55 p-5 md:p-7">
        <SectionTitle eyebrow={isPartner ? 'Partner outreach' : isNetwork ? 'Relationship building' : 'Outreach directo'} title="A quién contactar ahora" description={isPartner
          ...
          : isNetwork
            ? 'Perfiles seleccionados por afinidad profesional y actividad pública. El mensaje busca conversación real y reconocimiento mutuo, no convertirlos en leads.'
      ...
      <section className="mt-12">
        <SectionTitle eyebrow={isPartner ? 'Partner accounts' : isNetwork ? 'Professional context' : 'Target accounts'} title={isPartner ? 'Partners recomendados' : isNetwork ? 'Dónde se mueve esta red' : 'Empresas objetivo'} description={isPartner
          ...
          : isNetwork
            ? 'La organización es contexto, no objetivo comercial. Sirve para entender en qué ecosistema trabaja cada persona y por qué merece la pena seguirla.'
        ...
        {activeCompanies.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center text-sm text-slate-500">No hay cuentas activas en este motor.</div> : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {activeCompanies.slice(0, 36).map((company) => <article key={company.company_id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.035)] transition hover:border-indigo-200 hover:shadow-md">
            ...
            {company.recommended_service && <div className="mt-3 rounded-xl bg-emerald-50 px-3 py-2 text-xs text-emerald-800"><strong>{isPartner ? 'Gap a complementar:' : isNetwork ? 'Afinidad:' : 'Servicio prioritario:'}</strong> {company.recommended_service}</div>}
      ...
      {(opportunities.length > 0 || meetings.length > 0) && <section className="mt-12">
        <SectionTitle eyebrow={isNetwork?'Relación real':'Pipeline real'} title={isPartner ? 'Conversaciones, reuniones y acuerdos' : isNetwork ? 'Conversaciones y reuniones' : 'Oportunidades y reuniones'} description="Este bloque solo aparece cuando existe actividad real; no mostramos paneles vacíos como relleno." />
