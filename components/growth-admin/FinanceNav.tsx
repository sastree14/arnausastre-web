...
const modules = [
  ...
  { href:'/growth-admin/finance/expenses', label:'Gastos', description:'Manual + detección desde Gmail' },
  { href:'/growth-admin/finance/cash', label:'Caja y banco', description:'Cobros, pagos y conciliación' },
  { href:'/growth-admin/finance/history', label:'Histórico', description:'Cliente/proveedor, cobros y pagos' },
  ...
  { href:'/growth-admin/finance/integrations', label:'Integraciones', description:'Perfil fiscal, Drive, Gmail y Google' },
]

export default function FinanceNav({active}:{active:string}){
  return <nav className="mb-8 grid gap-2 sm:grid-cols-2 xl:grid-cols-4 2xl:grid-cols-8">
    {modules.map((item)=>{
