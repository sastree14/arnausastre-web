...
export default function LiveAdminForms(){
  ...
  useEffect(()=>{
    ...
    const onTerminal=(event:Event)=>{
      ...
      if(active){
        setButton(active.button,detail.status==='completed'?'Completado ✓':'Error · revisar',false)
        delete active.form.dataset.liveBusy
        activeForms.delete(detail.taskId)
        if(detail.status==='completed')window.setTimeout(()=>restoreButton(active.button),1200)
      }
      router.refresh();restoreScroll(scrollY)
