...
type ResizeState = { id: string; startX: number; startY: number; originX: number; originY: number; originW: number; originH: number } | null
...
const QUICK_PRESETS: Array<{ label: string; description: string; template: VisualTemplateKey; mode: PublicationMode }> = [
  ...
  { label: 'Métrica / resultado', description: 'Un dato protagonista con contexto.', template: 'metric_focus', mode: 'text_with_visual' },
  { label: 'Proceso / framework', description: 'Pasos conectados y lectura secuencial.', template: 'process_steps', mode: 'text_with_visual' },
  { label: 'Comparación', description: 'Dos marcos enfrentados con dirección visual.', template: 'comparison_split', mode: 'text_with_visual' },
  { label: 'Artículo / hero', description: 'Portada para web o artículo de LinkedIn.', template: 'article_editorial', mode: 'text_with_visual' },
  { label: 'Statement', description: 'Hook visual dominante para detener el scroll.', template: 'image_only_statement', mode: 'visual_first' },
]

const palette = Object.values(SC_BRAND.colors)
...
function buildDesign(seed: VisualStudioContentSeed, template?: VisualTemplateKey, format?: VisualFormatKey, mode?: PublicationMode) {
  const key = template || (isArticleContentType(seed.content_type) ? defaultVisualTemplate(seed) : recommendTemplates(seed)[0] || defaultVisualTemplate(seed))
  const formatKey = format || defaultVisualFormat(seed)
  return enhanceVisualDesign(createDesignFromTemplate(key, formatKey, { ...seed, publication_mode: mode || seed.publication_mode }), seed)
}
...
function SvgElement({ element }: { element: VisualElement }) {
  ...
  if (element.kind === 'line') return <line x1={element.x} y1={element.y} x2={element.x + element.w} y2={element.y + element.h} stroke={element.stroke || SC_BRAND.colors.accentBlue} strokeWidth={element.strokeWidth || 3} opacity={element.opacity ?? 1} strokeLinecap="round" markerEnd={element.role === 'flow-arrow' ? 'url(#flow-arrow-head)' : undefined}/>
...
function Canvas({ design, selectedId, onSelect, onPointerDown, selection = true, svgRef }: { design: VisualDesign; selectedId?: string; onSelect?: (id: string) => void; onPointerDown?: (event: React.PointerEvent<SVGGElement>, element: VisualElement) => void; selection?: boolean; svgRef?: React.RefObject<SVGSVGElement | null> }) {
  const format = VISUAL_FORMATS[design.formatKey]
  const selected = design.elements.find((element) => element.id === selectedId)
  return <svg ref={svgRef} viewBox={`0 0 ${format.width} ${format.height}`} width={format.width} height={format.height} className="h-auto w-full bg-white"><defs><marker id="flow-arrow-head" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 z" fill={SC_BRAND.colors.accentBlue}/></marker></defs><rect x="0" y="0" width={format.width} height={format.height} fill={design.background}/>{design.elements.map((element) => <g key={element.id} onPointerDown={onPointerDown ? (event) => onPointerDown(event, element) : undefined} onClick={() => onSelect?.(element.id)} className={onPointerDown && !element.locked ? 'cursor-move' : ''}><SvgElement element={element}/>{selection && selected?.id === element.id && !element.hidden && <g data-selection="true"><rect x={element.x - 7} y={element.y - 7} width={element.w + 14} height={element.h + 14} fill="none" stroke="#38BDF8" strokeWidth={3} strokeDasharray="12 8" pointerEvents="none"/></g>}</g>)}</svg>
}

function PublicationPreview({ seed, design }: { seed: VisualStudioContentSeed; design: VisualDesign }) {
  const isArticle = isArticleContentType(seed.content_type)
  const visualVisible = design.publicationMode !== 'text_only'
  if (isArticle && seed.content_type !== 'linkedin_post') {
    return <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white text-slate-900 shadow-sm">{visualVisible && <Canvas design={design} selection={false}/>}<div className="p-5"><p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-indigo-600">{contentTypeLabel(seed)}</p><h3 className="mt-2 text-xl font-semibold">{seed.title}</h3><div className="mt-4 max-h-52 overflow-auto whitespace-pre-wrap text-sm leading-6 text-slate-600">{seed.body}</div></div></div>
  }
  return <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white text-slate-900 shadow-sm"><div className="flex items-center gap-3 p-4"><div className="flex h-10 w-10 items-center justify-center rounded-full bg-slate-950 text-xs font-semibold text-white">AS</div><div><p className="text-sm font-semibold">Arnau Sastre · SC-Analytics</p><p className="text-[11px] text-slate-500">LinkedIn preview</p></div></div>{visualVisible && <Canvas design={design} selection={false}/>}<div className="p-4"><p className="whitespace-pre-wrap text-sm leading-6 text-slate-700">{design.publicationMode === 'image_only' ? 'SC-Analytics · Comprender antes de construir.' : seed.body}</p></div></div>
}
...
export default function VisualStudioClientV3({ content, savedDesigns, initialContentId, returnTo = '/growth-admin/content#publications' }: Props) {
  ...
  function setDirect(next: VisualDesign) { designRef.current = next; setDesign(next) }
  function snapshot() { undoStack.current = [...undoStack.current.slice(-59), cloneDesign(designRef.current)]; redoStack.current = [] }
  function mutate(fn: (current: VisualDesign) => VisualDesign, record = true) { if (record) snapshot(); setDirect(fn(designRef.current)) }
  function updateElement(id: string, changes: Partial<VisualElement>, record = true) { mutate((current) => ({ ...current, elements: current.elements.map((element) => element.id === id ? { ...element, ...changes } : element) }), record) }
  function undo() { const previous = undoStack.current.pop(); if (!previous) return; redoStack.current.push(cloneDesign(designRef.current)); setDirect(previous); setNotice({ tone: 'info', text: 'Cambio deshecho.' }) }
  function redo() { const next = redoStack.current.pop(); if (!next) return; undoStack.current.push(cloneDesign(designRef.current)); setDirect(next); setNotice({ tone: 'info', text: 'Cambio rehecho.' }) }
  ...
  async function addImage(file: File) {
    ...
    const width = Math.round(format.width * .38)
    const element: VisualElement = { ...base, id: `illustration-${Date.now()}`, role: 'illustration', src: dataUrl, locked: false, x: Math.round(format.width * .56), y: Math.round(format.height * .33), w: width, h: Math.round(format.height * .38) }
    mutate((current) => ({ ...current, elements: [...current.elements, element] }))
    setSelectedId(element.id)
    setNotice({ tone: 'success', text: 'Imagen añadida dentro del diseño. Puedes moverla y redimensionarla.' })
  }
  ...
  function moveSelected(dx: number, dy: number) {
    ...
    updateElement(selected.id, { x: Math.max(0, Math.min(format.width - selected.w, selected.x + dx)), y: Math.max(0, Math.min(format.height - selected.h, selected.y + dy)) })
  }

  function pointerPoint(event: React.PointerEvent<SVGElement>) {
  ...
  function startDrag(event: React.PointerEvent<SVGGElement>, element: VisualElement) {
    ...
    const point = pointerPoint(event); snapshot(); setDrag({ id: element.id, startX: point.x, startY: point.y, originX: element.x, originY: element.y })
  }

  function handleMove(event: React.PointerEvent<SVGSVGElement>) {
    if (!drag) return
    const element = designRef.current.elements.find((row) => row.id === drag.id); if (!element) return
    const point = pointerPoint(event)
    updateElement(element.id, { x: Math.round(Math.max(0, Math.min(format.width - element.w, drag.originX + point.x - drag.startX))), y: Math.round(Math.max(0, Math.min(format.height - element.h, drag.originY + point.y - drag.startY))) }, false)
  }
  ...
  useEffect(() => {
    function onKeyDown(event: KeyboardEvent) {
      ...
      if (mod && event.key.toLowerCase() === 's') { event.preventDefault(); void saveDesign(false); return }
  ...
  async function serializeSvg() {
    ...
    await Promise.all(images.map(async (image) => {
      ...
      try { const response = await fetch(href); if (!response.ok) return; const blob = await response.blob(); const dataUrl = await new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result || '')); reader.onerror = reject; reader.readAsDataURL(blob) }); image.setAttribute('href', dataUrl) } catch { /* optional image */ }
    }))
  ...
  async function exportPng() {
    ...
    try {
      ...
      const context = canvas.getContext('2d'); if (!context) throw new Error('Canvas 2D no disponible')
  ...
  async function saveDesign(asTemplate: boolean) {
    ...
    try {
      ...
      const response = await fetch('/api/growth-admin/visual-studio/save', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ designId: current.designId, contentId: asTemplate ? null : contentId, name: current.name, templateKey: current.templateKey, formatKey: current.formatKey, publicationMode: current.publicationMode, design: current, isTemplate: asTemplate }) })
    ...
    } catch (error) { setNotice({ tone: 'error', text: error instanceof Error ? error.message : 'No se pudo guardar.' }) } finally { setBusy('') }
  }
  ...
  async function attach() {
    ...
    try {
      ...
      const current = designRef.current
      const response = await fetch('/api/growth-admin/visual-studio/apply', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ designId: current.designId, contentId, name: current.name, templateKey: current.templateKey, formatKey: current.formatKey, publicationMode: current.publicationMode, design: current, pngDataUrl }) })
      if (!response.ok) throw new Error(await response.text())
      const result = await response.json()
      setDirect({ ...current, designId: result.designId, assetPath: result.assetPath, status: 'attached' })
      setNotice({ tone: 'success', text: 'Visual adjuntado. El preview y la publicación ya usan este diseño.' })
    } catch (error) { setNotice({ tone: 'error', text: error instanceof Error ? error.message : 'No se pudo adjuntar.' }) } finally { setBusy('') }
  }
  ...
  return <main className="min-h-screen bg-[#050816] text-slate-100">
    <header className="sticky top-0 z-30 border-b border-slate-800 bg-slate-950/95 px-5 py-4 backdrop-blur">
      <div className="mx-auto flex max-w-[1900px] flex-col gap-3 xl:flex-row xl:items-center xl:justify-between"><div className="flex items-center gap-3"><a href={returnTo} className="rounded-lg border border-slate-700 px-3 py-2 text-xs text-slate-300">← Volver</a><img src={SC_BRAND.logos.white} alt="SC-Analytics" className="h-11 w-44 object-contain object-left"/><div><p className="text-[10px] font-semibold uppercase tracking-[.18em] text-sky-300">Sistema visual privado · V3</p><h1 className="text-xl font-semibold">Visual Studio</h1></div></div><div className="flex flex-wrap gap-2"><button onClick={undo} className="rounded-lg border border-slate-700 px-3 py-2 text-xs">↶</button><button onClick={redo} className="rounded-lg border border-slate-700 px-3 py-2 text-xs">↷</button><button onClick={() => saveDesign(false)} disabled={Boolean(busy)} className="rounded-lg border border-slate-700 px-3 py-2 text-xs">Guardar</button><button onClick={() => saveDesign(true)} disabled={Boolean(busy)} className="rounded-lg border border-slate-700 px-3 py-2 text-xs">Guardar plantilla</button><a href={`/growth-admin/preview/${encodeURIComponent(contentId)}`} className="rounded-lg border border-slate-700 px-3 py-2 text-xs">Ver publicación</a><button onClick={attach} disabled={Boolean(busy)} className="rounded-lg bg-white px-4 py-2 text-xs font-semibold text-slate-950">{busy === 'attach' ? 'Adjuntando…' : 'Adjuntar visual'}</button></div></div>
    </header>
    ...
    <div className="mx-auto grid max-w-[1900px] 2xl:grid-cols-[330px_minmax(620px,1fr)_380px]">
      ...
      <aside className="border-l border-slate-800 bg-slate-950/70 p-5">
        ...
        {selected ? <div className="mt-5 space-y-4"><div className="flex gap-2"><button onClick={duplicateSelected} disabled={selected.role === 'brand' || selected.role === 'flow-arrow'} className="rounded-lg border border-slate-700 px-3 py-2 text-xs disabled:opacity-30">Duplicar</button><button onClick={removeSelected} disabled={selected.role === 'brand' || selected.role === 'flow-arrow'} className="rounded-lg border border-rose-900 px-3 py-2 text-xs text-rose-300 disabled:opacity-30">Borrar</button></div>
          ...
          {selected.kind === 'logo' && selected.role === 'brand' && <><label className="block text-[10px] uppercase tracking-wide text-slate-500">Color del logo<input type="color" value={selected.color || (design.background === SC_BRAND.colors.navy ? SC_BRAND.colors.white : SC_BRAND.colors.ink)} onChange={(event) => updateElement(selected.id, { color: event.target.value })} className="mt-1 h-10 w-full rounded border border-slate-700 bg-slate-900"/></label><div><p className="text-[10px] uppercase tracking-wide text-slate-500">Tamaño del logo</p><div className="mt-1 grid grid-cols-3 gap-2">{[['Base', .9], ['Grande', 1.08], ['XL', 1.25]].map(([label, factor]) => <button key={String(label)} onClick={() => { const f = Number(factor); const width = Math.round(Math.min(format.width * .45, Math.max(format.width * .24, format.width * .32 * f))); updateElement(selected.id, { x: Math.max(44, Math.round(Math.min(format.width, format.height) * .055)), y: Math.max(44, Math.round(Math.min(format.width, format.height) * .055)), w: width, h: Math.round(width * .28) }) }} className="rounded-lg border border-slate-700 px-2 py-2 text-[10px]">{label}</button>)}</div></div></>}
          ...
          <div className="grid grid-cols-2 gap-2">{(['x', 'y', 'w', 'h'] as const).map((key) => <label key={key} className="text-[10px] uppercase text-slate-500">{key.toUpperCase()}<input type="number" value={Math.round(selected[key])} disabled={selected.role === 'brand' || selected.role === 'flow-arrow'} onFocus={snapshot} onChange={(event) => updateElement(selected.id, { [key]: Number(event.target.value) }, false)} className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-900 px-2 py-2 text-sm text-white disabled:opacity-40"/></label>)}</div>
          {['text', 'metric', 'tag'].includes(selected.kind) && <label className="block text-[10px] uppercase tracking-wide text-slate-500">Color texto<input type="color" value={selected.color || SC_BRAND.colors.textOnDark} onChange={(event) => updateElement(selected.id, { color: event.target.value })} className="mt-1 h-10 w-full rounded border border-slate-700 bg-slate-900"/></label>}
          {['rect', 'tag'].includes(selected.kind) && <label className="block text-[10px] uppercase tracking-wide text-slate-500">Relleno<input type="color" value={selected.fill || SC_BRAND.colors.navyPanel} onChange={(event) => updateElement(selected.id, { fill: event.target.value })} className="mt-1 h-10 w-full rounded border border-slate-700 bg-slate-900"/></label>}
          <label className="block text-[10px] uppercase tracking-wide text-slate-500">Opacidad · {Math.round((selected.opacity ?? 1) * 100)}%<input type="range" min="0.1" max="1" step="0.05" value={selected.opacity ?? 1} onChange={(event) => updateElement(selected.id, { opacity: Number(event.target.value) })} className="mt-2 w-full"/></label>
        </div> : <p className="mt-5 rounded-xl border border-dashed border-slate-800 p-4 text-center text-xs text-slate-600">Selecciona un elemento para editarlo.</p>}
      </aside>
    </div>
  </main>
}
