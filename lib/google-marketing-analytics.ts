...
export async function syncGa4(days=365){
  ...
  try{
    ...
    const [totals,acquisition,events,dailyTotals,dailyEvents]=await Promise.all([
      ...
      googleJson(`https://analyticsdata.googleapis.com/v1beta/properties/${propertyId}:runReport`,accessToken,{method:'POST',body:JSON.stringify({dateRanges:[dateRange],dimensions:[{name:'eventName'}],metrics:[{name:'eventCount'}],dimensionFilter:{filter:{fieldName:'eventName',inListFilter:{values:['contact_click','contact_form_submitted','newsletter_subscribe','discovery_call_click','calendly_open','calendly_booked']}}},limit:'100'})}),
      ...
      googleJson(`https://analyticsdata.googleapis.com/v1beta/properties/${propertyId}:runReport`,accessToken,{method:'POST',body:JSON.stringify({dateRanges:[dateRange],dimensions:[{name:'date'},{name:'eventName'}],metrics:[{name:'eventCount'}],dimensionFilter:{filter:{fieldName:'eventName',inListFilter:{values:['contact_click','contact_form_submitted','newsletter_subscribe','discovery_call_click','calendly_open','calendly_booked']}}},limit:'100000'})}),
    ])
