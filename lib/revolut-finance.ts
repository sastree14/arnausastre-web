...
async function connection() {
  const rows=await queryGrowthTable<Json>('integration_connections',{tenant_id:'eq.sc-analytics',provider:'eq.revolut',account_type:'eq.business',order:'updated_at.desc',limit:'1'},{cacheSeconds:0})
  return rows[0] || null
}
...
export async function saveInitialRevolutConnection(token:Json) {
  ...
  const accounts=await revolutFetch(accessToken,'/accounts') as unknown as Json[]
  ...
  const changes={
    ...
    scopes:['READ'],metadata:{accounts_count:accounts.length,last_connected_at:now},updated_at:now,
  }
...
function primaryLeg(tx:Json, accountId:string) {
  ...
  return legs.find((leg:Json)=>String(leg.account_id||'')===accountId) || legs[0] || null
}
...
async function proposeMatch(bankTx:Json) {
  ...
  if(!match) return null
  const reconciliationId=`recon_${randomUUID().replaceAll('-','').slice(0,16)}`
  await insertGrowthRow('finance_reconciliations',{reconciliation_id:reconciliationId,tenant_id:TENANT_ID,bank_transaction_id:bankTx.bank_transaction_id,entity_type:'expense',entity_id:match.expense_id,matched_amount:amount,currency:bankTx.currency,confidence:0.85,status:'proposed',matched_by:'system',notes:'Unique exact amount match',created_at:new Date().toISOString()})
  await insertGrowthRow('finance_payments',{payment_id:`payment_${randomUUID().replaceAll('-','').slice(0,12)}`,tenant_id:TENANT_ID,expense_id:match.expense_id,payment_date:String(bankTx.booked_at||'').slice(0,10)||null,currency:bankTx.currency,amount,amount_eur:bankTx.amount_eur||null,direction:'outflow',method:'Revolut Business',reference:bankTx.reference||'',status:'recorded',review_status:'pending_review',source_system:'revolut',bank_transaction_id:bankTx.bank_transaction_id,reconciliation_status:'proposed',metadata:{reconciliation_id:reconciliationId,confidence:0.85},created_at:new Date().toISOString()})
  await updateGrowthRow('finance_bank_transactions','bank_transaction_id',String(bankTx.bank_transaction_id),{reconciliation_status:'proposed',updated_at:new Date().toISOString()})
  return {entity:'expense',id:match.expense_id,confidence:0.85}
}
...
export async function syncRevolutFinance(days=90) {
  ...
  for(const account of accounts) {
    const row=await upsertGrowthRow<Json>('finance_bank_accounts',{
      bank_account_id:`bank_${String(account.id).replace(/-/g,'').slice(0,16)}`,tenant_id:TENANT_ID,provider:'revolut',provider_account_id:String(account.id),display_name:String(account.name||`Revolut ${account.currency}`),currency:String(account.currency||'EUR'),status:String(account.state||'active'),metadata:{balance:account.balance,account_type:account.account_type,public:account.public},updated_at:new Date().toISOString(),created_at:new Date().toISOString()
  ...
  const txs=await revolutFetch(accessToken,`/transactions?from=${encodeURIComponent(from)}&count=1000`) as unknown as Json[]
  let stored=0,proposed=0
  for(const tx of txs) {
    for(const account of accounts) {
      const leg=primaryLeg(tx,String(account.id))
      if(!leg || String(leg.account_id||'')!==String(account.id)) continue
      const signed=Number(leg.amount||0)
      if(!signed) continue
      const providerId=`${tx.id}:${leg.leg_id||account.id}`
      const bankId=`banktx_${Buffer.from(providerId).toString('base64url').replace(/[^a-zA-Z0-9]/g,'').slice(0,24)}`
      const row=await upsertGrowthRow<Json>('finance_bank_transactions',{
        bank_transaction_id:bankId,tenant_id:TENANT_ID,bank_account_id:accountByProvider.get(String(account.id))?.bank_account_id||null,provider:'revolut',provider_transaction_id:providerId,booked_at:tx.created_at||null,completed_at:tx.completed_at||null,direction:signed>=0?'inflow':'outflow',currency:String(leg.currency||account.currency||'EUR'),amount:Math.abs(signed),amount_eur:String(leg.currency||account.currency)==='EUR'?Math.abs(signed):null,fx_rate:null,counterparty_name:String(tx.merchant?.name||leg.counterparty?.name||leg.description||''),reference:String(tx.reference||leg.description||''),status:String(tx.state||'completed'),raw_payload:tx,reconciliation_status:'unmatched',updated_at:new Date().toISOString(),created_at:new Date().toISOString()
      },'tenant_id,provider,provider_transaction_id')
      if(row){ stored+=1; if(row.reconciliation_status==='unmatched'){const match=await proposeMatch(row); if(match) proposed+=1} }
    }
  }
